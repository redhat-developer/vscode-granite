...
export class AiAssistantConfigurator {
    ...
    async configureAssistant() {
        const model = {
            ...
            contextLength: this.request.metadata.contextLength || 8192,
            systemMessage: this.request.metadata.systemMessage || 'You are a helpful AI assistant. You are a cautious assistant. You carefully follow instructions. You are helpful and harmless and you follow ethical guidelines and promote positive behavior.'
        } as Model;
        ...
        const existing = models.find((m) => model.provider === m.provider && model.apiBase === m.apiBase);
        ...
        if (updateConfig) {
            ...
            return vscode.window.showInformationMessage(`${model.model} added to ${configFile}`);
        }
    }
}
